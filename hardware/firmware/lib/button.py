...
class Button:
    ...
    def __init__(
        ...
        """
        Initialize a button with a GPIO pin, name and an Event to signal when the button is pressed.
        Button should be connected between the GPIO pin and GND, an internal pull-up resistor is enabled.
        Call wait_for_press() to start watching for button presses and set the event when a press is detected.
        """
        self.logger = uLogger(f"Button {gpio_pin}")
        self.logger.info(f"Initializing button '{button_name}' on GPIO pin {gpio_pin}")
        self.gpio = gpio_pin
