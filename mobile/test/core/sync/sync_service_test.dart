...
import 'package:simple_runner/core/sync/sync_service.dart';
...
Future<AuthService> _signedInAuthService(FakeApiClient api) async {
  ...
  final service = AuthService(apiClient: api, storage: const FlutterSecureStorage());
...
Future<AuthService> _signedOutAuthService() async {
  FlutterSecureStoragePlatform.instance = TestFlutterSecureStoragePlatform({});
  return AuthService(apiClient: FakeApiClient(), storage: const FlutterSecureStorage());
}
...
void main() {
  test('runFinished uploads a pending record and fetches analysis', () async {
    final api = FakeApiClient()
      ..getAnalysisHandler = ({required baseUrl, required token, required serverRunId}) async =>
          const AnalysisDto(status: 'done', result: {'distance_meters': 5012.0});
  ...
  test('a retryable failure (network) marks the record failed(retryable: true)', () async {
    final api = FakeApiClient()
      ..uploadRunHandler = ({
        required baseUrl,
        required token,
  ...
  test('retryNow retries a permanently-failed record on explicit user action', () async {
    var shouldFail = true;
    final api = FakeApiClient()
      ..uploadRunHandler = ({
        required baseUrl,
        ...
        if (shouldFail) throw const ApiRejectedException('too big', statusCode: 413);
    ...
    service.runFinished();
    await pumpEventQueue();
    expect((await store.listAll()).single.syncStatus, isA<SyncStatusFailed>());
  ...
  test('a 401 marks the auth service signed-out and keeps the record pending', () async {
    final api = FakeApiClient()
      ..uploadRunHandler = ({
        required baseUrl,
        required token,
        ...
        required gpxFile,
          ...
          throw const ApiUnauthorizedException('token revoked');
    final store = FakeRunStore()..seed(_record());
    final auth = await _signedInAuthService(api);
    final connectivity = FakeConnectivityMonitor();
    final service = SyncService(
    ...
    expect((await auth.currentState()).serverUrl, 'https://runner.example.com');
  });

  test('not signed in: the queue stays pending and nothing is uploaded', () async {
    final api = FakeApiClient();
    final store = FakeRunStore()..seed(_record());
  ...
  test('offline: the queue is left untouched until connectivity returns', () async {
    final api = FakeApiClient();
    final store = FakeRunStore()..seed(_record());
    final auth = await _signedInAuthService(api);
  ...
  test('oldest-first: two pending records upload in startedAt order', () async {
    ...
    final store = FakeRunStore()
      ...
      ..seed(_record(clientRunId: 'earlier', startedAt: DateTime.utc(2026, 1, 1)));
    final auth = await _signedInAuthService(api);
    final connectivity = FakeConnectivityMonitor();
    final service = SyncService(
      apiClient: api,
    ...
    expect(api.uploadCalls.map((s) => s.clientRunId).toList(), ['earlier', 'later']);
  });
}
